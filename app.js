/* NewfoundLUG Bulk 2026 – Parts Browser
   - Loads data.json (generated from your PDF)
...
function applyFilters(resetPage=true){
  ...
  filtered = DATA.filter(r=>{
    ...
    const hay = normalize(r.element_id) + " " + normalize(r.description) + " " + normalize(r.sub_group);
...
init().catch(err=>{
  console.error(err);
  results.innerHTML = `<div class="card"><div class="cardBody"><div class="desc">Failed to load data.json. If you're opening this file directly, run a tiny local server (instructions below).</div>
